...
export default function StadiumInformationStep1() {
  ...
  const validateFields = () => {
    ...
    // Location validation
    if (!location) {
      newErrors.location = "Vui lòng chọn vị trí sân tập";
    }
    ...
    if (validFields.length === 0) {
      newErrors.fields = "Vui lòng thêm ít nhất một sân";
    }
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      const step1Data = {
        ...
        // Don't send googleMap - let backend handle the migration
        phone,
        email,
        website,
        otherContacts,
  ...
  const toggleSport = (sportId: number) => {
    setSelectedSports((prev) => {
      const newSelected = prev.includes(sportId)
        ...
        : [...prev, sportId];
      // Clear error when sports are selected
      if (newSelected.length > 0 && errors.sports) {
    ...
    <View className="flex-1 bg-white">
      ...
      >
        ...
        >
          ...
          <View className="mb-3">
            ...
            >
              <Text className={location ? "text-black" : "text-gray-400"}>
                {location ? location.address : "Chọn vị trí sân tập"}
              </Text>
          ...
          <View className="mb-3">
            <Text className="text-base font-medium text-gray-700 mb-2">
              Tên các sân <Text className="text-red-500">*</Text>
            </Text>
            <DynamicInputList
              items={fields}
              onItemsChange={setFields}
              placeholder="Ví dụ: Sân A1, Sân B2, Court 1..."
              maxLength={50}
            />
            {errors.fields && (
              <Text className="text-red-500 text-sm mt-1">{errors.fields}</Text>
            )}
          </View>
          ...
          <View className="flex-row items-center mb-3">
            <View className="flex-1 mr-2">
              ...
              >
                <Text
                  className={startTimeDate ? "text-black" : "text-gray-400"}
                >
                  {startTimeDate
                    ? `${startTimeDate
                        .getHours()
