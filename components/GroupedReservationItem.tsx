...
const GroupedReservationItem: React.FC<GroupedReservationItemProps> = ({
  group,
  onPress,
  showActionButton = true,
}) => {
  const { representative, startTime, endTime, reservations } = group;
  ...
  const getUserInitial = () => {
    const name = representative.user?.fullName;
    if (name) {
      ...
      <View className="flex-1">
        <View className="flex-row items-center justify-between mb-1">
          <Text className="font-InterBold text-gray-900 text-base">
            {representative.user?.fullName || "Khách hàng"}
          </Text>
          <Text
        ...
        <Text className="text-sm text-gray-600 mb-1">
          {representative.sport || "Thể thao"}: Sân{" "}
          {representative.courtNumber || "N/A"}
        </Text>
        ...
        {reservations.length > 1 && (
          <Text className="text-xs text-blue-600 mb-1">
            ({reservations.length} khung giờ liên tiếp)
          </Text>
        )}

        <View className="flex-row items-center justify-between">
          <Text className="text-xs text-gray-500">
            {new Date(representative.date).toLocaleDateString("vi-VN")}
          </Text>
...
export default GroupedReservationItem;
